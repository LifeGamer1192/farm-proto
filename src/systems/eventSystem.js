...
export function onSeasonChange(game, season) {
  ...
  // α37: once per year on the first frame of winter, the largest colony
  // group declares war on the smallest if it has crossed the
  // WAR_DECLARE_POP_THRESHOLD. Skipped while a war is already in
  // progress, or while only one group exists.
  if (season === 'winter') maybeDeclareWar(game);
  maybeBirth(game);
