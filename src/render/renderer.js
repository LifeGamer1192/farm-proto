...
// α37 combat — arrow + damage-number overlays live in combatRender so
// the main renderer doesn't have to grow new combat-visual methods.
import { drawArrows, drawDamageNumbers } from './combatRender.js';
...
export class Renderer {
  ...
  draw(scene) {
    ...
    const now = scene.clock || 0;
    drawArrows(this.ctx, scene.arrows, proj, elevAt, now);
    drawDamageNumbers(this.ctx, scene.damageNumbers, proj, elevAt, now);
  }
